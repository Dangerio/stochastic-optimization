...
* ```particle_swarm (f, dim, x_lower, x_upper, alpha, beta, gamma, M, L, tolerance)``` - функция, реализующая метод роения частиц для минимизации некоторой вещественнозначной функции многих переменных.
    ...
    * **Вход**: 
      ...
      * `tolerance` -- минимальная разность значений функции $f$, служит критерием останова.
...
>> [y, x] = particle_swarm(F, opt_set, 0.95, 0.2, 0.2, 100, 10^6, 0)
